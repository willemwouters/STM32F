...
*/
# if !(defined (_POSIX_SOURCE) || defined(STM32) || defined (_WINSOCK_H) || defined (_WINSOCKAPI_) || defined (__USE_W32_SOCKETS)) 
...
/* We use a macro for fd_set so that including Sockets.h afterwards
   can work.  */
//WWtypedef	struct _types_fd_set {
//WW	fd_mask	fds_bits[howmany(FD_SETSIZE, NFDBITS)];
//WW} _types_fd_set;

//WW#define fd_set _types_fd_set

//WW#  define	FD_SET(n, p)	((p)->fds_bits[(n)/NFDBITS] |= (1L << ((n) % NFDBITS)))
//WW#  define	FD_CLR(n, p)	((p)->fds_bits[(n)/NFDBITS] &= ~(1L << ((n) % NFDBITS)))
//WW#  define	FD_ISSET(n, p)	((p)->fds_bits[(n)/NFDBITS] & (1L << ((n) % NFDBITS)))
//WW#  define	FD_ZERO(p)	(__extension__ (void)({ \
     size_t __i; \
     char *__tmp = (char *)p; \
