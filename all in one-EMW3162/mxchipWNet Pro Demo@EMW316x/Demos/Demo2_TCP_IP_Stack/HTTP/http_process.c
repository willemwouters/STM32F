...
#define APP_INFO          "mxchipWNet Pro Demo: Web server demo 2."
...
static void HandleHttpClient(int index)
{
  ...
  msleep(100); // sleep 200 ms, just incase http request fregment.
  memset(httpRequest, 0x0, HTTP_DATA_MAX_LEN);
...
static void send_http_data(int index, char *data, int len)
{
  ...
  u32 end_time = mico_get_time()+5000; // max wait for 5 seconds.
  ...
  while(SendCount < len)
  {
    ...
    if (end_time<mico_get_time()) // timeout
      return;
    SendCount+=NumAtOnce;
  }	  
}
...
static void get_settings_post(int index, char *postdata, int len)
{
  ...
  int bytes_received, content_len = 0;
  ...
  while(content_len > 0){
    bytes_received = recv(index, end_pos, HTTP_DATA_MAX_LEN, 0 );
