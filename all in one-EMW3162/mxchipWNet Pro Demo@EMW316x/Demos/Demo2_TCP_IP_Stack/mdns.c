  ...
  for ( a = 0; a < htons(iter->header->question_count); ++a )
  {
    ...
    case RR_TYPE_PTR:
      if ( available_services != NULL ){
        ...
        if ( dns_compare_name_to_string( &name, SERVICE_QUERY_NAME, __FUNCTION__, __LINE__ ) ){
          int b = 0;
          debug_out("UDP multicast test: Recv a SERVICE QUERY request.\r\n");
        ...
        // else check if its one of our records
          ...
          for ( b = 0; b < available_service_count; ++b ){
            if ( dns_compare_name_to_string( &name, available_services[b].service_name, __FUNCTION__, __LINE__ )){
              ...
              debug_out("UDP multicast test: Recv a SERVICE Detail request.\r\n");
              if(dns_create_message( &response, 512 )){
    ...
    if (!question_processed ){
      //debug_out("UDP multicast test: Recv a mDNS request.\r\n");
                               ...
                               dns_question_t* question, dns_message_iterator_t* source )
{
  ...
  case RR_TYPE_A:
    if ( dns_compare_name_to_string( name, available_services->hostname, __FUNCTION__, __LINE__) ){				
      debug_out("UDP multicast test: Recv RR_TYPE_A.\r\n");
  ...
  default:
    debug_out("UDP multicast test: Request not support type: %d.---------------------\r\n", question->question_type);
  }
}
...
static int dns_compare_name_to_string( dns_name_t* name, const char* string, const char* fun, int line )
{
  ...
  debug_out("UDP multicast test: CMP called by %s@%d.\r\n", fun, line );
  char *temp;
  ...
  while ( !finished )
  {
    ...
    while ( *buffer & 0xC0 )
    {
      ...
      debug_out("UDP multicast test: Recv a compressed name.\r\n");
    }
    ...
    debug_out("UDP multicast test: Recv a name: %s.\r\n", temp);
    free(temp);
...
static void mdns_send_message(int fd, dns_message_iterator_t* message )
{
  ...
  addr.s_port = 5353;
  debug_out("UDP multicast test: Send a mDNS respond!+++++++++++++++++++++++++++\r\n");
